...
function reactifyIcon(fileName) {
    ...
    fse.readFile(path.resolve(sourcePath, fileName), 'utf8', (err, data) => {
        ...
        let newData = data.substr(data.indexOf('<svg'))
                            .replace(/ xmlns="([a-zA-Z0-9\-\_:/\.]+\s*)+"/gmi, '')
        ...
        const viewBoxRegex = /(viewBox="0 0 ([0-9.]+) ([0-9.]+)")/gmi;
        ...
        if (viewBoxMatch != null) {
            ...
            newData = newData.replace(viewBoxMatch[0], viewBoxMatch[0] + ` width={${isSquare ? 'size' : 'calculatedWidth'}} height={${isSquare ? 'size' : 'calculatedHeight'}}`)
        }
        ...
        const kebabCaseProps = newData.match(/\w+-\w+(=|:)/gmi);
        if (kebabCaseProps) {
        ...
        const fillAndStrokeRegex = /((stroke)|(fill))="([a-zA-Z0-9#]+)"/gmi;
        ...
        output.push('');
        output.push(`const ${newFileName}: React.FC<{ ${isSquare ? sizeType : widthType}${colorsType}, className?: string }> = ({ ${isSquare ? sizeParameters : widthParameters}${colorsParameter}, className }) => ${isSquare ? '(' : '{'}`);
        if (!isSquare) {
            output.push(`    const { calculatedWidth, calculatedHeight } = calculateDimensions(width, height, ${width}, ${height});`)
