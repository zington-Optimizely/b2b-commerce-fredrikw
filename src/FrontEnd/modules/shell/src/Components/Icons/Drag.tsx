...
const Drag: React.FC<{ width?: number, height?: number, color1?: string, className?: string }> = ({ width = undefined, height = undefined, color1 = '#4D4D4D', className }) => {
    const { calculatedWidth, calculatedHeight } = calculateDimensions(width, height, 9, 15);
    return <svg focusable="false" className={className} viewBox="0 0 9 15" width={calculatedWidth} height={calculatedHeight}>
