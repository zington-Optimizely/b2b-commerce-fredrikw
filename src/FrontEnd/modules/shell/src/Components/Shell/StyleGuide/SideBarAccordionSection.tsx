...
const SideBarAccordionSection: React.FunctionComponent<{ title: string, inPopover?: boolean }> = ({ title, children, inPopover }) => {
    ...
    return (
        <AccordionSection
            title={title}
            titleTypographyProps={{ variant: "h3", css: css`
                margin-bottom: 0;
                margin-top: 0;
            ` }}
            headerProps={{
                css: css<AccordionSectionHeaderProps>`
                    ...
                    button {
                        ...
                        border: 0;
                        background: ${({ expanded, theme }) => expanded ? theme.colors.common.accentContrast : "transparent"};
                        > span {
                            color: ${({ expanded, theme }) => expanded ? theme.colors.common.accent : theme.colors.text.main};
                            font-weight: ${({ expanded }) => expanded ? "bold" : 300};
                        }
                        &:focus {
                            border: 0;
                            background: ${({ expanded, theme }) => expanded ? theme.colors.common.accentContrast : "rgba(0, 0, 0, 0.3)"};
                            padding: 9.5px ${paddingValue}px;
                            > span {
                                color: ${({ expanded, theme }) => expanded ? theme.colors.primary.main : theme.colors.text.main};
                            }
                        }
                    }
            ...
            panelProps={{
                css: css`
                    ...
                    background-color: ${({ theme }) => inPopover ? theme.colors.common.background : theme.colors.common.accent};
