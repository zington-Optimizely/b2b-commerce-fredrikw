...
class MainNavigationItem extends React.Component<ThemeProps<BaseTheme> & MainNavigationItemProps> {
    ...
    render() {
        const { styles, link, index, theme: { breakpoints: { maxWidths } } } = this.props;
        ...
        let menuLink = (
            ...
            >
                🐶 {link.title}{isCascading && downIcon}
            </Link>
        );
        if (isMega || link.url === "") {
            menuLink = (
                <Link
                    ...
                    {...styles.menuItem}
                >
                    🐶 {link.title}{(isCascading || isMega) && downIcon}
                </Link>
            );
        }
        ...
        if (isCascading && link.children) {
            ...
            if (link.maxDepth) menuProps.maxDepth = link.maxDepth;
            menuItem = <Menu
        ...
        if (isMega) {
            ...
            >
                <GridContainer {...styles.megaMenuGridContainer} offsetProps={{ ...styles.megaMenuGridContainer?.offsetProps, as: "ul" }}>
                    {link.children!.filter(child => !child.excludeFromNavigation).map((child, index) =>
                        // eslint-disable-next-line react/no-array-index-key
                        <GridItem width={12 / link.numberOfColumns! as GridWidths} key={index} {...styles.megaMenuGridItem} as="li">
                            <Link {...styles.megaMenuHeading} href={child.url} onClick={this.closePopover}>{child.title}</Link>
                                {child.children
                                    && <ul>
                                        {child.children.filter(grandChild => !grandChild.excludeFromNavigation).map((grandChild, index) =>
                                            // eslint-disable-next-line react/no-array-index-key
                                            <li key={index}><Link
                                                {...styles.megaMenuLink}
