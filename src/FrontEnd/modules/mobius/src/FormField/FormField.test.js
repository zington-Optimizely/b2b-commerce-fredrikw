...
describe('FormField', () => {
    ...
    describe('label', () => {
        test('renders when prop is provided', () => {
            props = {
                label: 'props.label', inputId: 4, descriptionId: 18, formInput: <input/>,
            };
        ...
        test('small styles applied', () => {
            props = {
                label: 'props.label', inputId: 4, descriptionId: 18, sizeVariant: 'small', formInput: <input/>,
            };
        ...
        test('default styles applied', () => {
            props = {
                label: 'props.label', inputId: 4, descriptionId: 18, sizeVariant: 'default', formInput: <input/>,
            };
            const label = wrapper().find(FormField).find('label');
    ...
    describe('required adds an asterisk to the label', () => {
        test('when not disabled', () => {
            const label = () => wrapper().find(FormField).find('label').getDOMNode();
            props = {
                label: 'props.label', required: true, disabled: false, inputId: 4, descriptionId: 18, formInput: <input/>,
            };
            expect(label().innerHTML).toBe(`${props.label} *`);
        });
        ...
        test('but not when disabled', () => {
            const label = () => wrapper().find(FormField).find('label').getDOMNode();
            props = {
                label: 'props.label', required: true, disabled: true, inputId: 4, descriptionId: 18, formInput: <input/>,
            };
            expect(label().innerHTML).toBe(props.label);
        });
    });
    ...
    test('renders the hint text when provided', () => {
        props = {
            hint: 'props.hint', inputId: 4, descriptionId: '18-description', formInput: <input/>,
        };
    ...
    test('renders error text when provided', () => {
        props = {
            error: 'props.error', inputId: 4, descriptionId: '18-description', formInput: <input/>,
        };
        const hintText = wrapper().find(FormField).find('[id$="18-description"] span');
