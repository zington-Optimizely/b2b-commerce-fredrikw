...
const tabGenerator = (title, children) => (<Tab tabKey={title} key={title} headline={title}>{children}</Tab>);
...
const baseTabs = [
    tabGenerator('moose', typeText('some info')),
    tabGenerator('cat', typeText('some other info')),
];
...
describe('TabGroup', () => {
    ...
    test('renders each child tab passed to it', () => {
        tabs = baseTabs;
        tabs.push(<Tab headline={<Icon src={Mail} />} tabKey='Mail' key='Mail'>{typeText('Mail lobby')}</Tab>);
    ...
    test('renders the children of each child tab passed to it', () => {
        ...
        tabs = [
            tabGenerator('complext', complexChildren),
            tabGenerator('cat', typeText(text[3])),
        ];
    ...
    test('visually hides all but the currently selected tab content', () => {
        tabs = baseTabs;
        wrapper().find('[data-id="tabContent"]').children().forEach((a) => {
