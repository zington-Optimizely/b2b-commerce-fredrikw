        ...
        describe('appropriately applies theme overrides', () => {
            ...
            describe('css', () => {
                test('instance with no theme applies instance', () => {
                    ...
                    expect(wrapper().find("div[data-id='checkbox']")).toHaveStyleRule('border-bottom', '1px solid rebeccapurple');
                });
                test('theme with no instance applies theme', () => {
                    theme = { checkbox: { defaultProps: { css: "border-bottom: 1px dashed red;"}}};
                    expect(wrapper().find("div[data-id='checkbox']")).toHaveStyleRule('border-bottom', '1px dashed red');
                });
                test('theme and instance applies instance', () => {
                    props = { css: "border-bottom: 1px solid rebeccapurple;" };
                    theme = { checkbox: { defaultProps: { css: "border-bottom: 1px dashed red;"}}};
                    expect(wrapper().find("div[data-id='checkbox']")).toHaveStyleRule('border-bottom', '1px solid rebeccapurple');
                    expect(wrapper().find("div[data-id='checkbox']")).not.toHaveStyleRule('border-bottom', '1px dashed red');
                });
            });
