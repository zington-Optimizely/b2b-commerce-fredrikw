function getThroughPathArray(object = {}, pathArray: (string | number | undefined)[], defaultValue: any) {
    if (!pathArray) return object;
    try {
        if (pathArray.length === 1) return (object as any)[pathArray[0] as any];
 ...
 */
export default function get(object: any = {}, path?: string | number | undefined | (string | number | undefined)[], defaultValue?: any) {
    if (path === "" || path === null || path === undefined) return defaultValue;
    if (Array.isArray(path)) return getThroughPathArray(object, path, defaultValue);
