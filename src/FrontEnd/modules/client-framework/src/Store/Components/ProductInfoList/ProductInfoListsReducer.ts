...
const reducer = {
    ...
    "Components/ProductInfoLists/CompleteLoadProductInfoList": (draft: Draft<ProductInfoListState>, action: {
        id: string,
    ...
    "Components/ProductInfoLists/UpdateProduct": (draft: Draft<ProductInfoListState>, action: {
        id: string,
        productId: string,
        qtyOrdered?: number,
    ...
    "Components/ProductInfoLists/CompleteLoadRealTimePricing": (draft: Draft<ProductInfoListState>, action: { realTimePricing: RealTimePricingModel, id: string }) => {
    ...
    "Components/ProductInfoLists/FailedLoadRealTimePricing": (draft: Draft<ProductInfoListState>, action: { id: string }) => {
        const productInfoList = draft.productInfoListById[action.id];
        if (!productInfoList) {
            throw new Error(`There was no productInfoList found for the id ${action.id}`);
        }
    ...
    "Components/ProductInfoLists/CompleteLoadRealTimeInventory": (draft: Draft<ProductInfoListState>, action: { realTimeInventory: RealTimeInventoryModel, id: string }) => {
        const productInfoList = draft.productInfoListById[action.id];
        if (!productInfoList) {
            throw new Error(`There was no productInfoList found for the id ${action.id}`);
        }
