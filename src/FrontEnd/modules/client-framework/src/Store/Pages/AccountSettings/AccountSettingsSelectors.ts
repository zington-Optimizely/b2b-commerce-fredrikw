...
export function hasChanges(state: ApplicationState) {
    const { accountSettings: { editingAccount, initialUseDefaultCustomer, useDefaultCustomer, initialShipToId, selectedShipToId } } = state.pages;
    const account = getCurrentAccountState(state).value;
    if (!account || !editingAccount) {
        return false;
    }

    return account.email !== editingAccount.email
