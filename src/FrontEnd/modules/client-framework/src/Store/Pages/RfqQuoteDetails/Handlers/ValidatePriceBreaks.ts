...
type HandlerType = HandlerWithResult<HasOnSuccess<boolean>, {
...
function shouldValidatePriceBreak(isJobQuote: boolean, quoteLine: QuoteLineModel, priceBreak: BreakPriceRfqModel): boolean {
    if (!isJobQuote) {
        return true;
    }

    return quoteLine!.pricingRfq!.priceBreaks!
        .slice()
...
function validatePrice(result: PriceBreakValidation, quoteLine: QuoteLineModel, priceBreak: BreakPriceRfqModel) {
    ...
    if ((quoteLine!.pricingRfq!.minimumPriceAllowed > 0 && priceBreak.price < quoteLine!.pricingRfq!.minimumPriceAllowed) || priceBreak.price < 0) {
...
function validateStartQty(result: PriceBreakValidation, priceBreaks: BreakPriceRfqModel[], maxQty: number) {
    ...
    if (index === 0) {
        ...
        if (priceBreaks[index].startQty <= priceBreaks[index - 1].startQty || priceBreaks[index].startQty < priceBreaks[index - 1].endQty) {
            result.invalidQty = true;
        }

        if ((index === priceBreaks.length - 1) && priceBreaks[priceBreaks.length - 1].startQty && maxQty > 0 && maxQty < priceBreaks[priceBreaks.length - 1].startQty) {
            result.invalidQty = true;
        }
    }
}
...
export const chain = [
    InitResult,
    ValidatePriceBreaks,
    DispatchCompleteValidatePriceBreaks,
