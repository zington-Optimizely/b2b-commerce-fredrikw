...
const reducer = {
    "Data/Budget/CompleteLoadBudgetCalendarCollection": (draft: Draft<BudgetState>, action: { collection: BudgetCalendarCollectionModel; }) => {
        if (!draft.budgetCalendar) {
            const currentYear = new Date().getFullYear();
            draft.budgetCalendar = action.collection.budgetCalendarCollection!.find(o => o.fiscalYear === currentYear)
        ...
        for (let i = 0; i < budgetEndPeriods.length; i += 1) {
            if ((i === budgetEndPeriods.length - 1)) {
                budgetEndPeriods[i] = getYearEnd(draft.budgetCalendar.fiscalYear, draft.budgetCalendar.fiscalYearEndDate!);
            } else {
    ...
    "Pages/BudgetManagement/CompleteSaveBudgetConfiguration": (draft: Draft<BudgetState>, action: {
        budgetCalendar: BudgetCalendarModel;
    }) => {
        draft.budgetCalendar = { ...draft.budgetCalendar, ...action.budgetCalendar };
    },
    "Pages/BudgetManagement/SetBudgetCalendar": (draft: Draft<BudgetState>, action: { budgetCalendar?: BudgetCalendarModel; }) => {
        draft.budgetCalendar = action.budgetCalendar ? { ...draft.budgetCalendar, ...action.budgetCalendar } : undefined;
    },
    "Pages/BudgetManagement/SetBudgetEndPeriods": (draft: Draft<BudgetState>, action: { budgetEndPeriods?: Date[]; }) => {
        draft.budgetEndPeriods = action.budgetEndPeriods;
    },
    "Pages/BudgetManagement/SetBudgetPeriodType": (draft: Draft<BudgetState>, action: { budgetPeriodType?: BudgetPeriodType; }) => {
        draft.budgetPeriodType = action.budgetPeriodType;
    },
    "Pages/BudgetManagement/SetCustomBudgetPeriodNumber": (draft: Draft<BudgetState>, action: { customBudgetPeriodNumber?: number; }) => {
        draft.customBudgetPeriodNumber = action.customBudgetPeriodNumber;
    },
    "Pages/BudgetManagement/SetDisplayedWidgetName": (draft: Draft<BudgetState>, action: { displayedWidgetName: BudgetWidgetNames; }) => {
        draft.displayedWidgetName = action.displayedWidgetName;
    },
    "Pages/BudgetManagement/UpdateLoadParameter": (draft: Draft<BudgetState>, action: { parameter: GetBudgetApiParameter; }) => {
        draft.getBudgetParameter = { ...draft.getBudgetParameter, ...action.parameter };
    },
    "Pages/BudgetManagement/UpdateLoadMaintenanceInfoParameter": (draft: Draft<BudgetState>, action: { parameter: GetBudgetApiParameter; }) => {
        draft.maintenanceInfo = undefined;
        draft.getMaintenanceInfoParameter = { ...draft.getMaintenanceInfoParameter, ...action.parameter };
    },
    "Pages/BudgetManagement/UpdateMaintenanceInfo": (draft: Draft<BudgetState>, action: { maintenanceInfo?: BudgetModel; }) => {
