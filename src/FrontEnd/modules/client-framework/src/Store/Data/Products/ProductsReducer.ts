...
const reducer = {
    ...
    "Data/Products/CompleteLoadProducts": (draft: Draft<ProductsState>, action: { parameter: object, collection: ProductCollectionModel }) => {
        const { parameter, collection } = action;

        setDataViewLoaded(draft, parameter, collection, collection => collection.products!, (product: ProductModel) => {
    ...
    "Data/Products/CompleteLoadProduct": (draft: Draft<ProductsState>, action: { model: ProductModel, path?: string }) => {
        if (action.path) {
