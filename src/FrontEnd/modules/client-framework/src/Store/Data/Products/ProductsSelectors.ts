...
import { GetProductCollectionApiV2Parameter, GetRelatedProductCollectionApiV2Parameter } from "@insite/client-framework/Services/ProductServiceV2";
...
import { ProductSettingsModel } from "@insite/client-framework/Types/ApiModels";
...
export function getProductsDataView(state: ApplicationState, getProductsParameter?: GetProductCollectionApiV2Parameter | GetRelatedProductCollectionApiV2Parameter) {
...
export function hasEnoughInventory(state: ApplicationState, productContext: ProductContextModel) {
    const { product, productInfo: { inventory } } = productContext;
    const { fulfillmentMethod } = getSession(state);
    const { productSettings } = getSettingsCollection(state);
    const allowBackOrder = fulfillmentMethod === FulfillmentMethod.PickUp ? productSettings.allowBackOrderForPickup : productSettings.allowBackOrderForDelivery;

    return allowBackOrder || !product.trackInventory || (inventory && inventory.qtyOnHand && inventory.qtyOnHand > 0);
}
...
export function getComputedVariantProduct(parentProductState: ReturnType<typeof getProductStateByPath>, variantProductState: ReturnType<typeof getProductState>) {
