...
import { BrandModel, BrandCategoryCollectionModel, BrandProductLineCollectionModel, BrandCollectionModel, BrandCategoryModel, BrandProductLineModel } from "@insite/client-framework/Types/ApiModels";
...
const reducer = {
    ...
    "Data/Brands/CompleteLoadBrands": (draft: Draft<BrandsState>, action: { parameter: object, collection: BrandCollectionModel }) => {
        setDataViewLoaded(draft, action.parameter, action.collection, collection => collection.brands!);
    },
    "Data/Brands/BeginLoadCategories": (draft: Draft<BrandsState>, action: { parameter: GetBrandCategoriesApiParameter }) => {
        draft.brandCategoryDataView[getDataViewKey(action.parameter)] = {
            isLoading: true,
            value: undefined,
        };
    },
    "Data/Brands/CompleteLoadCategories": (draft: Draft<BrandsState>, action: { parameter: GetBrandCategoriesApiParameter, collection: BrandCategoryCollectionModel }) => {
    ...
    "Data/Brands/BeginLoadProductLines": (draft: Draft<BrandsState>, action: { parameter: GetBrandProductLinesApiParameter }) => {
        draft.brandProductLineDataView[getDataViewKey(action.parameter)] = {
            isLoading: true,
            value: undefined,
        };
    },
    "Data/Brands/CompleteLoadProductLines": (draft: Draft<BrandsState>, action: { parameter: GetBrandProductLinesApiParameter, collection: BrandProductLineCollectionModel }) => {
    ...
    "Data/Brands/CompleteLoadBrandByPath": (draft: Draft<BrandsState>, action: { path: string, brand?: BrandModel }) => {
