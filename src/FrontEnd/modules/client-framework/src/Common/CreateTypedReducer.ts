...
type SimpleActionHandlers<T> = {
    [P in keyof T]: // For each property on the object...
        Action<P> // ...create an action typed as the property name...
        & // ...combine that type with the result of a condition:...
        ((T[P] extends (...args: any) => any ? // ...if the property is a function (needed to satisfy the constraint on Parameters)...
        Parameters<T[P]>[1] : {} // ...get the type of the second parameter, otherwise provide an empty object type;...
        ) extends undefined ? {} : undefined); // ...if the above expression returns undefined (no second parameter) return {}, otherwise return undefined;...
...
type ActionUnion<T> =
    SimpleActionHandlers<T>[keyof SimpleActionHandlers<T>] // Rotate the output of SimpleActionHandlers into a discriminated union...
...
export function createTypedReducerWithImmer<State extends object, Handlers extends ImmerHandlerFunctionMap<State>>(
    ...
    return (state = initialState, action) => {
        ...
        return deepFreezeObject(produce(state, draft => {
